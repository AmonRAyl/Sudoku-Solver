...
function fetchSudoku() {
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      if (aux["difficulty"] === 'Easy' || aux["difficulty"] === 'Medium') {
          let coordinates = [];
            for (let i = 0; i < 9; i++) {
                for (let j = 0; j < 9; j++){
                    if (aux["value"][i][j] != 0){
                        let string= "g["+i+"]["+j+"].setActualValue('"+aux["value"][i][j]+"');";
                        coordinates.push(string);
                    }
                }
            }

          let newContent = `Problem:\n${matrixToString(aux["value"])}\nSolution:\n${matrixToString(aux["solution"])}\n\n${coordinates.join('\n')}`;
        ...
        console.log('Retrying for an easy/medim difficulty Sudoku...');
        fetchSudoku();
      }
    })
    .catch(error => {
